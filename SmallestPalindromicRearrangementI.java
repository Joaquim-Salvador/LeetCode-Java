...
class SmallestPalindromicRearrangementI{
    public String smallestPalindrome(String s) {
        StringBuilder menorPallindromo = new StringBuilder();
        ...
        for(int i = 0; i < 26; i++){
            ...
            if(qtdAparece[i] > 0){
                while(j < qtdAparece[i] / 2) { 
                    menorPallindromo.append((char) ('a' + i));
                    j++;
                }
            }
        ...
        for(int i = menorPallindromo.length() - 1; i >= 0; i--){
            if(i == menorPallindromo.length() - 1 && s.length() % 2 != 0){
                menorPallindromo.append(letraMeio);
            }
            menorPallindromo.append(menorPallindromo.charAt(i));

        }
        
        return menorPallindromo.toString();
    }
